...
| **Programmable outward API** (drive it from other agents) | ~ Agent SDK | ❌ | ❌ | ❌ | ❌ | ✅ npm SDK + MCP server (workspaces, agents, tasks, hosts) | ~ `@highflame/codeoid-core` + `@highflame/codeoid-protocol` on npm, but transport-level; no control-plane MCP server |
